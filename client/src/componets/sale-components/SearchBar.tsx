...
export const SearchBar = ({ onSearch }: SearchBarProps) => {
  ...
  // Handle search function when clicking the search button
  const handleSearch = (e:any) => {
    e.preventDefault(); // Prevent the form from submitting (if necessary)
    // console.log('Searching for:', searchValue);
    // You can now use searchValue for the actual search logic
    onSearch(searchValue);
  };

  // Handle the input value change
  const handleInputChange = (e:any) => {
    setSearchValue(e.target.value); // Update the state as the user types
  };
